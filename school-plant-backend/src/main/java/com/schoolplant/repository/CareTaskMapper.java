...
public interface CareTaskMapper extends BaseMapper<CareTask> {
    ...
    @Select("SELECT * FROM care_tasks WHERE adopter_id = #{userId} AND status = #{status} ORDER BY due_date ASC")
    ...
    @Select("SELECT COUNT(*) FROM care_tasks WHERE adopter_id = #{userId} AND status = 'COMPLETED'")
    int countCompletedByUser(@Param("userId") Long userId);

    @Select("SELECT COUNT(*) FROM care_tasks WHERE adopter_id = #{userId}")
